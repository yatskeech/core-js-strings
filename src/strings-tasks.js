...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < (str ? str.length : 0); i += 1) {
...
function countVowels(str) {
  const strValue = str.toUpperCase();
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < strValue.length / 2; i += 1) {
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.length; i += 1) {
    if (sentence[i] === ' ') {
      if (lengthWord > currentMax) {
        currentMax = lengthWord;
        currentIndex = i - lengthWord;
      }
  ...
  return sentence.slice(currentIndex, currentIndex + currentMax);
}
...
function invertCase(str) {
  return str
    .split('')
    .map(function toInvertCase(char) {
...
function encodeToRot13(str) {
  return str
    ...
    .map(function encodeChar(char) {
      ...
      if (char >= 'A' && char <= 'M') {
        codeChar = String.fromCharCode(char.charCodeAt(0) + 13);
...
function getCardId(value) {
  let valueCard = value.slice(0, -1);
  let suitCard = value.slice(-1);
  ...
  return suitCard * 13 + valueCard - 1;
}
